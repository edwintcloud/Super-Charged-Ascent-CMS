...
if (!isset($msg_send) && !isset($msg_read) && $folder != "options") {
	...
	echo "<tr>\n<td align='left' width='100%' class='tbl4'><a href='".FUSION_SELF."?msg_send=0'>".$locale['401']."</a></td>\n";
	echo "<td width='1%' class='tbl4' style='white-space:nowrap;font-weight:".($folder=="inbox"?"bold":"normal")."'><a href='".FUSION_SELF."?folder=inbox'>".$locale['402']." [".$bdata['inbox_total']."/".$msg_settings['pm_inbox']."]</a></td>\n";
	echo "<td width='1%' class='tbl4' style='white-space:nowrap;font-weight:".($folder=="outbox"?"bold":"normal")."'><a href='".FUSION_SELF."?folder=outbox'>".$locale['403']." [".$bdata['outbox_total']."/".$msg_settings['pm_sentbox']."]</a></td>\n";
	echo "<td width='1%' class='tbl4' style='white-space:nowrap;font-weight:".($folder=="archive"?"bold":"normal")."'><a href='".FUSION_SELF."?folder=archive'>".$locale['404']." [".$bdata['archive_total']."/".$msg_settings['pm_savebox']."]</a></td>\n";
	echo "<td width='1%' class='tbl4' style='white-space:nowrap;font-weight:".($folder=="options"?"bold":"normal")."'><a href='".FUSION_SELF."?folder=options'>".$locale['425']."</a></td>\n";
	echo "</tr>\n</table>\n";
	if ($total_rows) {
		echo "<table cellpadding='0' cellspacing='1' width='100%' class='tbl-border'>\n";
		echo "<tr>\n<td class='tbl6'>".$locale['405']."</td>\n";
		echo "<td width='1%' class='tbl6' style='white-space:nowrap'>".($folder != "outbox" ? $locale['406'] : $locale['421'])."</td>\n";
		echo "<td width='1%' class='tbl6' style='white-space:nowrap'>".$locale['407']."</td>\n</tr>\n";
		while ($data = dbarray($result)) {
  ...
  $result = dbquery(
    "SELECT m.*, u.user_id, u.user_name, u.user_avatar
    FROM ".$db_prefix."messages m
    LEFT JOIN ".$db_prefix."users u ON m.message_from=u.user_id
    WHERE message_to='".$userdata['user_id']."'
    AND message_id='$msg_read'"
      );
	...
	if (dbrows($result)) {
		...
		echo "<table cellpadding='0' cellspacing='1' width='100%' class='tbl-border'>\n<tr>\n";
		  echo "<tr><td rowspan='3' align='right' width='1%' class='tbl1'>";
if ($data['user_avatar'] != "") {
   echo "<img src='".IMAGES."avatars/".$data['user_avatar']."' alt='".$locale['567']."'>";
		...
		echo "<tr>\n<td colspan='3' class='tbl1'>".nl2br(parseubb($message_message))."</td>\n</tr>\n";
		echo "</table>\n";
		echo "<table border='0' cellpadding='0' cellspacing='0' width='100%'>\n";
