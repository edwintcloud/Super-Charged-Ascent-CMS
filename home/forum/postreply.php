...
if ($_POST['validation'] != "" && $_POST['validation'] == $_POST['validation_answer'])
			...
			if (dbcount("(*)", "posts", "forum_id='$forum_id' AND thread_id='$thread_id' AND post_message='$message' AND post_datestamp+84600>".time())) {
				...
				if ($attach['name'] != "" && !empty($attach['name']) && is_uploaded_file($attach['tmp_name']) && $attach['size'] <= $settings['attachmax']) {
					...
					if (in_array($attachext, $attachtypes)) {
						...
						$result = dbquery("INSERT INTO ".$db_prefix."forum_attachments VALUES('', '$thread_id', '$newpost_id', '$attachname', '$attachext', '".$attach['size']."')");
	...
	if (isset($_POST['postreply']) && $invalid) echo "<div style='text-align:center'><br />You must answer the maths question correctly for your forum entry to be added. This is to prevent bots from spamming the forum. We apologise for the inconvenience.<br /><br /></div>";
	
	if (!isset($_POST['previewreply']) || $invalid ) {
		$subject = "";
		$message = "";
...
<td class='tbl2'><input type='text' name='subject' value='"; if($invalid) { echo $subject2; } else { $subject; } echo"' class='textbox' maxlength='255' style='width:250px'>".$locale['465']."</td>
</tr>
<tr>
